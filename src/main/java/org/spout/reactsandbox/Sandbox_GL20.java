...
public class Sandbox_GL20 {
	...
	private static final Map<CollisionBody, OpenGL20Model> shapes = new HashMap<CollisionBody, OpenGL20Model>();
	private static final Map<CollisionBody, OpenGL20Model> aabbs = new HashMap<CollisionBody, OpenGL20Model>();
	// Input
	...
	// Rendering
	private static OpenGL20Renderer renderer = new OpenGL20Renderer();
	...
	private static CollisionBody addBody(CollisionBody body) {
		...
		final OpenGL20Model aabbModel = MeshGenerator_GL20.generateWireCuboid(new Vector3(1, 1, 1));
	...
	private static void removeBody(final CollisionBody body) {
		final OpenGL20Model shapeModel = shapes.remove(body);
		renderer.removeModel(shapeModel);
		shapeModel.destroy();
		final OpenGL20Model aabbModel = aabbs.remove(body);
	...
	private static void updateBodies() {
		for (Entry<CollisionBody, OpenGL20Model> entry : shapes.entrySet()) {
			final CollisionBody body = entry.getKey();
			final OpenGL20Model shape = entry.getValue();
			final OpenGL20Model aabbModel = aabbs.get(body);
			final AABB aabb = body.getAABB();
	...
	private static void setupRenderer() {
		...
		solidProgram.setVertexShaderSource(Sandbox_GL20.class.getResourceAsStream("/solid.vert"));
		solidProgram.setFragmentShaderSource(Sandbox_GL20.class.getResourceAsStream("/solid.frag"));
		...
		wireframeProgram.setVertexShaderSource(Sandbox_GL20.class.getResourceAsStream("/wireframe.vert"));
		wireframeProgram.setFragmentShaderSource(Sandbox_GL20.class.getResourceAsStream("/wireframe.frag"));
	...
	private static void deploy() throws Exception {
		...
		for (String nativeLib : nativeLibs) {
			...
			if (!nativeFile.exists()) {
				FileUtils.copyInputStreamToFile(Sandbox_GL20.class.getResourceAsStream("/" + nativeLib), nativeFile);
			}
		}
